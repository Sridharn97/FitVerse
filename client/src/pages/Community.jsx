...
const Community = () => {
    ...
    const handleCreate = () => {
        if (!title || !content || !user)
            return;
        addPost({ userId: user.id, userName: user.name, title, content, category });
        setTitle("");
        setContent("");
        setOpen(false);
    };
    const handleComment = (postId) => {
        if (!commentTexts[postId]?.trim() || !user)
            return;
        addComment(postId, { userId: user.id, userName: user.name, content: commentTexts[postId] });
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <Avatar className="h-10 w-10">
                      <AvatarFallback className="bg-primary text-primary-foreground">{post.userName.charAt(0).toUpperCase()}</AvatarFallback>
                    </Avatar>
                    <div>
